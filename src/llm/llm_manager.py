...
# from src.config.config import DEFAULT_LLM_PROVIDER
...
class LLMManager:
    ...
    async def generate_response(self, prompt: str, temperature: float = 0.7, 
                              max_attempts: int = 3, chat_history: Optional[List] = None) -> Tuple[str, str]:
        """
        ...
        # Prepare full prompt with chat history context if provided
        full_prompt = prompt
        ...
        while attempts < max_attempts and len(providers_tried) < len(self.providers):
            ...
            try:
                ...
                response = await provider.generate_response(full_prompt, temperature)
