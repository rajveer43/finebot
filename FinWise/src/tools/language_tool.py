...
async def detect_and_translate(text: str, target_language: str = 'en') -> str:
    """
    Utility function to detect the language of text and translate it to the target language.
    ...
    try:
        ...
        detection_result = await lang_tool._detect_language(text)
        source_language = detection_result["detected_language"]
        ...
        # Translate the text
        translation_result = await lang_tool._translate_text(
            text=text, 
        ...
        logging.error(f"Error in detect_and_translate: {str(e)}")
